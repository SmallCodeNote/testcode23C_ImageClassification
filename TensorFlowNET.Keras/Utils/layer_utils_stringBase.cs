...
{
   public  static class layer_utils_byString
    {
        public static string get_summary(Model model, int line_length = -1, float[] positions = null)
        {
            ...
            print.Add(string.Join("", range(line_length).Select(x => "_")));
            print.Add(print_row(to_display, positions_int));
            print.Add(string.Join("", range(line_length).Select(x => "=")));

            foreach (var (i, layer) in enumerate(model.Layers))
            {
                if (sequential_like)
                    print.Add(print_layer_summary(layer, positions_int));
                else
                    print.Add(print_layer_summary_with_connections(layer, positions_int, relevant_nodes));
            ...
            print.Add($"Total params: {trainable_count + non_trainable_count}");
            print.Add($"Trainable params: {trainable_count}");
            print.Add($"Non-trainable params: {non_trainable_count}");
            print.Add(string.Join("", range(line_length).Select(x => "_")));
        ...
        static string print_row(string[] fields, int[] positions)
        {
            var line = "";
        ...
        static string print_layer_summary(ILayer layer, int[] positions)
        {
            var name = layer.Name;
        ...
        static string print_layer_summary_with_connections(ILayer layer, int[] positions, List<INode> relevant_nodes)
        {
            List<string> print = new List<string>();
            ...
            print.Add(print_row(fields, positions));
